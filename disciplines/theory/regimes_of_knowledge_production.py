...
Theory:
    ...
    research products takes place.

Krige, J. and Pestre, D. (eds) (1997), Science in the Twentieth Century, Amsterdam:
Harwood Academic Publishers.

Data:
    process of establishing research programmes :
    dissemination types : how disseminate?
    amount of written work generated : compared to others.

    movement between worlds : are they moving back and forth?
    dissemination practices : where work is disseminate?
    trips and their duration : are they visiting? are they staying in other discipline?

    profession heterogeneity
    technicians : are they recognisable?
    practitioners : are they recognisable?
    advisors : are they recognisable?
    institutional diversity : are institutions diverse?
    patents : are they available?

    importance of methodology : can be found based on theoretical books or references to methodologies
    practice descriptions :  are they standardised? from books and or literature.
    research focuses :
    origins of research focuses :
...
def is_regime_disciplinary(dsc):
    ...
    Checks whether regime is disciplinary.
...
def is_regime_transitory(dsc):
    ...
    Checks whether regime is transitory.
...
def is_regime_utilitarian(dsc):
    ...
    Checks whether regime is utilitarian.
...
def is_regime_transverse(dsc):
    ...
    Theory:
        ...
        influence of other groups. This regime is transverse for at least three reasons: (i) the
    ...
    Vars:
        importance_of_methodology (float) : 
        standartization_of_practice (float) :
        research_focuses_have_multiple_origins (float) :
        dissemination_spaces_are_diverse (float):
        related_to_other_regimes (float) :
...
def regimes_for_knowledge_production(dsc): #returns a dictionary of regimes and associated values
    """ Getting regime for knowledge production of a discipline
