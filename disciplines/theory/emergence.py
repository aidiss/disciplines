...
Does some things like:
Checks a stage of development
Also contains multiple functions for looking for evidence of stage of emergence.


Kinds of functions:
1. for details mentioned in each stage a list. consists of (int) of functions
2. for detecting whether discipline fulfills criteria of a stage.
3. combining pairs
4. for combining all details and stages
...
def recreate_emerge(discipline):
	...
	add_node(author, author_data)
	add_node(publication, publication_data)
	add_node(event, event_data)
	add_node(concept, concept_data)

	add_edge(author, publication, {when="timestamp"})
	add_edge(person, vent, {when="timestamp"})
	add_edge(journal, book, {when="timestamp"})
	add_edge(person, person, {when="timestamp"})
	add_edge(concept, publication, {when="timestamp"})

# Or same with claimege
	def add_authorship(data, graph ,author, publication, data):
	...
	def add_organizer(data, graph, event, organizer, data):
		"""adds stuff to a graph,
